...
def lambda_handler(event, context):
    ...
    if "AWSCURRENT" in versions[token]:
        ...
        return 'something to return, couple times'
    ...
    if step == "createSecret":
        # Rotation can be done only per 24 hours
        exit_code = rotate_per_day(dynamodb_client, dynamodb_resource, arn)
        if exit_code == 0:
            response = service_client.update_secret_version_stage(SecretId=arn,VersionStage='AWSPENDING',RemoveFromVersionId=token)
            http_code = response['ResponseMetadata']['HTTPStatusCode']
            logger.error("Secret %s can be rotated once per day. AWSPending removed -> HTTP Status code: %s" % (arn, http_code))
            raise ValueError("Secret %s can be rotated once per day" % arn)
...
def rotate_per_day(dynamodb_client, dynamodb_resource, arn):
    
    secret_id = arn.split("/")[-1]
    print('secret id -----> ' + secret_id)
    
    dynamo_table = 'rds-compliance'
    list_table = dynamodb_client.list_tables()
    get_item = dynamodb_resource.batch_get_item(RequestItems={ dynamo_table: { 'Keys': [{ 'secret': secret_id }] } })

    table = dynamodb_resource.Table(dynamo_table)

    if dynamo_table not in list_table['TableNames']:
        print('Table ' + dynamo_table + ' does not exists!')
        # raise error

    if not get_item['Responses'][dynamo_table]:
        print('Secret item: ' + secret_id + ' does not exists in ' + dynamo_table + ', Value will be set to table.')
        table.put_item(Item={'secret': secret_id, 'limit': 'null'})

    get_secret = [ secret['secret'] for secret in get_item['Responses'][dynamo_table] if secret['secret'] == secret_id ]

    if secret_id not in get_secret:
        current_date = date.today().isoformat()
        table.put_item(Item={'secret': secret_id, 'limit': current_date})
    
    get_limit = [ limit['limit'] for limit in get_item['Responses'][dynamo_table] if limit['secret'] == secret_id ]

    date_limit = time.strptime(get_limit[0], "%Y-%m-%d")
    now = time.strptime(date.today().isoformat(), "%Y-%m-%d")
    
    if now > date_limit:
        current_date = date.today().isoformat()
        table.put_item(Item={'secret': secret_id, 'limit': current_date})
        print('You can rotate secret now')
        return 1
